...
int main(int argc, char *argv[])
{
	...
	//	cout << "sending count=" << count <<endl;
		...
		while(in) {
		     ...
		     in >> asset_price >> strike_price >> days_to_exp >> risk_free_rate >> standard_deviation;
                     if(in){
			     long control = 1;
			    // cout << " send in " << my_id << "  num_procs=" << num_procs << " control=" << control << endl;
			     MPI_Send(&control, 1, MPI_LONG, destination, 0, MPI_COMM_WORLD); 
			    
	//		     cout << " send in " << my_id << "  num_procs=" << num_procs << " to " << destination << endl;
	...
	//	     cout << " send in " << my_id << "  num_procs=" << num_procs << "terminate signal to " << i << endl;
	...
	//	  cout << " recv in " << my_id << "  num_procs=" << num_procs << " result=" << result << " from" << destination << endl;
                  of << result << endl; 
                  count--;
                  destination++; 
         ...
         /* do this work in any remaining processes */
		     ...
		     MPI_Recv(&control, 1, MPI_LONG, 0, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE); 
                     if(!control) {
	//	        cout << " recv in "<< my_id << "  num_procs=" << num_procs << "termination signal" << endl;
			break;
                     }
		     ...
		     double result = blackscholes(
			       ...
			       risk_free_rate,
			       standard_deviation);
              //cout << " calc..." << my_id << "  num_procs=" << num_procs << " result=" << result << endl;
